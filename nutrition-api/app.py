...
# ---------------------------------------------------------------------------
# AI — Plan d'entraînement généré par LLM (chantier 2 MSPR2)
# ---------------------------------------------------------------------------

WORKOUT_GOAL_LABELS_FR = {
    "weight_loss": (
        "perte de graisse / sèche — favorise les exercices polyarticulaires et le cardio "
        "modéré, déficit calorique, séances 45-60 min"
    ),
    "muscle_mass": (
        "prise de masse musculaire — hypertrophie, séries 8-12 reps avec charges modérées, "
        "volume élevé (3-5 séries par exercice), repos 60-90s, focus sur les grands groupes "
        "musculaires (jambes, dos, pecs)"
    ),
    "muscle_gain": (
        "prise de masse musculaire — hypertrophie, séries 8-12 reps avec charges modérées, "
        "volume élevé (3-5 séries par exercice), repos 60-90s"
    ),  # alias rétro-compatibilité
    "strength": (
        "renforcement musculaire / force pure — séries lourdes 3-6 reps, repos long 2-3 min, "
        "focus sur les mouvements de base (squat, soulevé de terre, développé)"
    ),
    "endurance": (
        "endurance et cardio — séances cardio progressives (course, vélo, rameur), HIIT, "
        "intervalles, peu de musculation lourde"
    ),
    "general_health": (
        "santé générale — équilibre cardio (2x/sem) et musculation légère (2x/sem), "
        "mobilité, exercices fonctionnels"
    ),
    "maintenance": (
        "maintien de la forme — programme équilibré sans surcharge, "
        "rotation cardio + renforcement"
    ),
}
...
class WorkoutPlanAIRequest(BaseModel):
    ...
    days_per_week: int = 3             # nombre de séances/semaine
    session_duration_min: int = 45     # durée par séance (min)
    equipment: list[str] = []          # ex. ["dumbbells", "mat"], vide = sans matos
    location: str = "home"             # home / gym / outdoor
    preferences: list[str] = []        # ex. ["cardio", "hiit"]
    limitations: list[str] = []        # ex. ["genou", "épaule droite"]
    recent_sessions: list[dict] = []   # historique pour rotation et progression


class WorkoutExercise(BaseModel):
    name: str
    sets: Optional[int] = None
    reps: Optional[str] = None     # "10-12" ou "30 secondes" pour cardio
    rest_seconds: Optional[int] = None
    notes: Optional[str] = None


class WorkoutSession_AI(BaseModel):
    day_label: str                 # "Jour 1 — Push", "Jour 2 — Cardio", etc.
    focus: str                     # "Haut du corps", "Cardio HIIT", "Jambes"
    estimated_duration_min: int
    estimated_calories: int
    warm_up: list[str]             # 2-3 lignes d'échauffement
    exercises: list[WorkoutExercise]
    cool_down: list[str]           # 2-3 lignes de retour au calme


class WorkoutPlanAIResponse(BaseModel):
    weekly_plan: list[WorkoutSession_AI]
...
async def workout_plan_ai(req: WorkoutPlanAIRequest):
    """
    Génère un plan d'entraînement hebdomadaire personnalisé via gpt-oss.
    Multi-critères : objectif, niveau, équipement, durée, préférences, limitations,
    historique récent (pour rotation et progression adaptative).
    """
    if not OPENROUTER_API_KEY:
        raise HTTPException(status_code=503, detail="OPENROUTER_API_KEY non configurée.")

    goal_fr = WORKOUT_GOAL_LABELS_FR.get(req.goal, req.goal)
    equipment_clause = (
        f"Équipement disponible : {', '.join(req.equipment)}." if req.equipment
        else "Aucun équipement (poids du corps uniquement)."
    )
    preferences_clause = (
        f"Activités appréciées : {', '.join(req.preferences)}." if req.preferences else ""
    )
    limitations_clause = (
        f"Limitations physiques : {', '.join(req.limitations)}. Évite tout exercice qui les sollicite."
        if req.limitations else ""
    )
    history_clause = ""
    if req.recent_sessions:
        recent_summary = "; ".join(
            f"{s.get('focus', '?')} le {s.get('date', '?')[:10]}"
            for s in req.recent_sessions[:5]
        )
        history_clause = (
            f"Séances récentes : {recent_summary}. "
            "Évite de répéter les mêmes muscles 2 jours consécutifs ; varie les focus."
        )

    system_prompt = (
        "Tu es un coach sportif francophone diplômé. Tu construis des programmes "
        "d'entraînement SÉCURISÉS, PROGRESSIFS et MOTIVANTS, adaptés au niveau "
        "et au matériel de l'utilisateur. Tu réponds UNIQUEMENT en JSON pur, "
        "SANS markdown, SANS texte avant ni après."
    )

    user_prompt = f"""Crée un plan d'entraînement HEBDOMADAIRE adapté à :
- Objectif : {goal_fr}
- Niveau : {req.level}
- Lieu : {req.location}
- {equipment_clause}
- {req.days_per_week} séances par semaine
- Durée cible par séance : {req.session_duration_min} minutes
- {preferences_clause}
- {limitations_clause}
- {history_clause}
...
{{
  "weekly_plan": [
    {{
      "day_label": "Jour 1 — Haut du corps",
      "focus": "Pectoraux, épaules, triceps",
      "estimated_duration_min": 45,
      "estimated_calories": 280,
      "warm_up": ["5 min de corde à sauter", "Rotations articulaires"],
      "exercises": [
        {{"name": "Pompes", "sets": 3, "reps": "10-12", "rest_seconds": 60, "notes": "Garde le dos droit"}},
        {{"name": "Développé haltères", "sets": 4, "reps": "8-10", "rest_seconds": 90}}
      ],
      "cool_down": ["Étirements pecs et triceps 5 min"]
    }}
  ],
  "progression_tips": "Phrase de conseil sur la progression (charges, répétitions, fréquence).",
  "rotation_note": "Comment varier la semaine suivante pour maintenir la motivation."
}}

Important :
- Adapte les sets/reps au niveau (débutant = sets 2-3, reps 8-12 ; avancé = sets 4-5, reps variables).
- Pour cardio/HIIT, mets reps = "30 secondes" et rest_seconds = 30.
- Échauffement et retour au calme OBLIGATOIRES.
- Exercices RÉALISTES en français, sans matériel sortant de l'équipement listé."""
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"JSON LLM invalide : {exc}. Réponse : {content[:300]}",
        )
