...
class UserProfile(models.Model):
    ...
    # --- Mini réseau social (MSPR 6.3/6.4) : panneau de contrôle utilisateur ---
    display_name = models.CharField(
        max_length=50, blank=True, help_text="Nom d'affichage public dans le flux social"
    )
    avatar = models.FileField(
        upload_to='avatars/', null=True, blank=True,
        help_text='Photo de profil (stockée via MEDIA_ROOT ; MinIO/S3 en production)'
    )
...
class Post(models.Model):
    """Publication du mini réseau social (flux unique — MSPR 6.3/6.4).

    Une publication contient un texte et/ou un média (photo/vidéo). Les likes
    et commentaires sont gérés par les modèles ``PostLike`` et ``PostComment``.
    """
    ...
    media = models.FileField(
        ...
        help_text='Photo ou vidéo associée à la publication',
    )
    media_type = models.CharField(
        max_length=10, blank=True,
        help_text="Type de média : 'image' ou 'video'",
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class PostLike(models.Model):
    """Like d'une publication (unique par utilisateur et publication)."""
