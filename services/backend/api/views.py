...
# ---------------------------------------------------------------------------
# Mini réseau social (MSPR 6.3/6.4) — flux, publications, likes, commentaires
# ---------------------------------------------------------------------------

def _media_type_from_upload(uploaded) -> str:
    """Devine 'image' / 'video' depuis le content-type ou l'extension."""
    if uploaded is None:
        return ''
...
class PostViewSet(viewsets.ModelViewSet):
    """Flux social unique : liste / création / suppression de publications,
    plus les actions ``like`` (toggle) et ``comments`` (liste / ajout)."""
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(
            ...
            media_type=_media_type_from_upload(media),
        )
    ...
    def comments(self, request, pk=None):
        """GET : liste des commentaires ; POST : ajout d'un commentaire."""
        post = get_object_or_404(Post, pk=pk)
...
class SocialProfileView(generics.RetrieveUpdateAPIView):
    """GET/PATCH /api/social/profile/ — panneau de contrôle de l'utilisateur
