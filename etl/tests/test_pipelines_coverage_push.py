...
class TestCoveragePush(unittest.TestCase):
    ...
    def test_pipeline_error_and_optional_sources_and_report(self):
        ...
        pd.read_csv(data_src / "diet_recommendations.csv").head(5).to_csv(tmp / "diet_recommendations.csv", index=False)
        pd.read_csv(data_src / "gym_members_exercise.csv").head(5).to_csv(tmp / "gym_members_exercise.csv", index=False)
        pd.read_csv(data_src / "daily_food_nutrition.csv").head(5).to_csv(tmp / "daily_food_nutrition.csv", index=False)
